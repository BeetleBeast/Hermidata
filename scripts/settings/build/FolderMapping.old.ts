import { type AnyNovelType, type AnyReadStatus, type Settings, type FolderMapping as FolderMappingType, type FolderRule, DEFAULT_NOVEL_TYPES, DEFAULT_READ_STATUSES } from "../../shared/types/index";
...
export class FolderMapping extends Build {
    ...
    private readonly FinishedMappingContainer = getElement<HTMLDivElement>('#folderMappingContainer');
    ...
    private buildFolderMappingForm(settings: Settings) {
        // build select
        const novelTypes = settings.ContentTypesAndStatuses.TYPE_OPTIONS;
        const readStatus = settings.ContentTypesAndStatuses.STATUS_OPTIONS;
    ...
    private async addNovelTypeFolder(newType: string | undefined) {
        ...
        settings.ContentTypesAndStatuses.TYPE_OPTIONS.push(newType);
        await this.dbRequest<Settings>('settings', 'put', { id: 'Settings', data: settings});
        this.resetForm(settings.ContentTypesAndStatuses.TYPE_OPTIONS, settings.ContentTypesAndStatuses.STATUS_OPTIONS);
    }
    private async addNovelStatusFolder(newStatus: string | undefined) {
        ...
        settings.ContentTypesAndStatuses.STATUS_OPTIONS.push(newStatus);
        await this.dbRequest<Settings>('settings', 'put', { id: 'Settings', data: settings});
        this.resetForm(settings.ContentTypesAndStatuses.TYPE_OPTIONS, settings.ContentTypesAndStatuses.STATUS_OPTIONS);
