...
import type { CurrentTab, Hermidata, novelType as NovelType , readStatus as ReadStatus } from './main';
...
let HermidataNeededKeys: { Page_Title: string, GoogleSheetURL: string, Past: Hermidata | null } = {
    Page_Title: '',
    GoogleSheetURL: '',
    Past: null,
}
let HermidataV3: Hermidata = {
    id: '',
    title: '',
    type: novelType[0] as NovelType,
    url: '',
    source: '',
    status: readStatus[0] as ReadStatus,
    chapter: {
        current: 0,
        latest: 0,
        history: [],
...
let rssPreloadPromise: Promise<{ notifications: { items: DocumentFragment; }; allItems: { header: DocumentFragment; items: DocumentFragment; }; }> | null = null;
let rssDOMCache = null;

let AllHermidata: { [s: string]: Hermidata; } | ArrayLike<Hermidata>;
let selectedIndex = -1;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    getElement('#isNewHermidata').textContent = HermidataNeededKeys.Past?.title ? '' : 'New Hermidata!';
    getElement("#Pagetitle").textContent = HermidataNeededKeys.Page_Title;
    getElement<HTMLInputElement>("#title").value =  HermidataNeededKeys.Past?.title || HermidataV3.title;
    getElement<HTMLInputElement>("#title_HDRSS").value = HermidataNeededKeys.Past?.title || HermidataV3.title;
    getElement<HTMLInputElement>("#Type").value = HermidataNeededKeys.Past?.type || HermidataV3.type;
    getElement<HTMLInputElement>("#Type_HDRSS").value = HermidataNeededKeys.Past?.type || HermidataV3.type;
    getElement<HTMLInputElement>("#chapter").value = String(HermidataV3.chapter.current);
    getElement<HTMLInputElement>("#url").value = HermidataV3.url;
    getElement<HTMLInputElement>("#status").value =  HermidataNeededKeys.Past?.status || HermidataV3.status;
    getElement<HTMLInputElement>("#date").value = new Intl.DateTimeFormat('en-GB').format(new Date()) || "";
    getElement<HTMLInputElement>("#tags").value = HermidataNeededKeys.Past?.meta?.tags.toString() || HermidataV3.meta.tags.toString();
    getElement<HTMLInputElement>("#notes").value = HermidataV3.meta.notes;
    FixTableSize()
    getElement("#save").addEventListener("click", async () => await saveSheet());
    getElement("#HDClassicBtn").addEventListener("click", (e) => openClassic(e));
    getElement("#HDRSSBtn").addEventListener('mouseenter', (e) =>  preloadRSS());
    ...
    getElement("#openFullPage").addEventListener("click", () => {
        try {
            ext.tabs.create({ url: HermidataNeededKeys.GoogleSheetURL });
        } catch (error) {
...
function removeKeysFromSync(key: string) {
...
// Get active tab info
function getCurrentTab(): Promise<Hermidata> {
...
function findByTitleOrAltV2(title: string, allData: { [key: string]: Hermidata }) {
...
function getChapterFromTitle(title: string, url: string) {
...
// Get GoogleSheet URL
function getGoogleSheetURL(): Promise<string> {
    return new Promise((resolve, reject) => {
        try {
 ...
 */
async function getHermidata(): Promise<Hermidata> {
try {
    // get all Hermidata
    ...
    let possibleObj: { [key: string]: Hermidata; }
...
async function migrateCopy(objs: any[]) {
    ...
    objs.sort((a: { lastUpdated: number; }, b: { lastUpdated: number; }) => new Date(b.lastUpdated || 0) - new Date(a.lastUpdated || 0));
    return objs[0] || {};
}
async function migrationSteps(obj1: { lastUpdated: number; date: number; }, obj2: { lastUpdated: number; date: number; }, options = {}) {
...
async function tryToFindByOtherMeans(possibleObj: ArrayLike<Hermidata> | { [s: string]: Hermidata; }) {
...
async function getHermidataViaKey(key: string): Promise<{} | Hermidata> {
 ...
 */
function isSameSeries(a: Hermidata, b: Hermidata) {
    if (!a || !b) return false;
 ...
 */
async function confirmMigrationPrompt(newer: { title: any; type: any; chapter: { current: any; }; meta: { notes: any; }; }, older: { type: any; chapter: { current: any; }; meta: { notes: any; }; }, options = {}) {
    try {
...
function deactivateother() {
    ...
    getElementAll(".HDClassic").forEach(a => {
        a.style.pointerEvents = 'none';
    });
    // deactivate links in HDRSS
    getElementAll(".HDRSS").forEach(a => {
        a.style.pointerEvents = 'none';
    });
    const classicCurrentActive = getElement(`#${'HDClassicBtn'}.${'active'}`);
    getElement(".HDRSS").style.opacity = classicCurrentActive ? 0 : 0.2;
    getElement(".HDClassic").style.opacity = classicCurrentActive ? 0.2 : 0;
}
function activateother() {
    ...
    getElement(".HDRSS").style.opacity = classicCurrentActive ? 0 : 1;
    getElement(".HDClassic").style.opacity = classicCurrentActive ? 1 : 0;
}
function customPrompt(msg: string | null, defaultInput: string) {
    return new Promise((resolve) => {
        const container = getElement('.promptSection');
        const input = getElement('.genericInput');
...
function customConfirm(msg: string | null) {
    return new Promise((resolve) => {
        const container = getElement('.promptSection');
        const label = getElement('.genericLabel');
        const btn1 = getElement('.genericButton1');
 ...
 */
async function migrateHermidataV5(newer: { title: any; type: any; id: string; url: any; source: any; status: any; chapter: { current: any; latest: any; history: any; lastChecked: any; }; rss: any; import: any; meta: { tags: any; notes: any; altTitles: any; }; }, older: { title: any; type: any; id: string; url: any; source: any; status: any; chapter: { current: any; latest: any; history: any; lastChecked: any; }; rss: any; import: any; meta: { tags: any; notes: any; altTitles: any; added: any; }; }, OLD_KEY = 'DEFAULT', NEW_KEY = 'DEFAULT') {
    // step 1. new key
    // re-make keys
    ...
    const mergeAltTitles = (mainTitle: any, ...altLists: any[][]) => {
        return [
            mainTitle,
...
function detectHashType(obj: { title: any; type: any; id: string; }) {
...
function getOldIDType(Obj: Hermidata) {
    const OLD_simpleHash = (str: string) => {
        let hash = 0, i, chr;
...
function returnHashedTitle(title: string | undefined,type: string | undefined) {
    return type 
...
function sheetUrlInput(resolve: { (value: string | PromiseLike<string>): void; (arg0: any): void; }, reject: { (reason?: any): void; (arg0: Error): any; }) {
...
function isValidGoogleSheetUrl(url: string) {
...
function trimTitle(title: string) {
    ...
    const parts = cleanstring.split(/ (?:(?:-+)|–|-|:|#|—|,|\|) /g).map((p: string) => p.trim()).filter(Boolean);
    ...
    const cleanTitleKeyword = (title: string) => {
        return title
    ...
    let filtered = parts
        .filter((p: string) => !readRegex.test(p))
        .filter((p: string) => !junkRegex.test(p))
        .filter((p: string) => !siteNameRegex.test(p))
        .filter((p: string) => !flexibleSiteNameRegex.test(p))
        .map((p: any) => cleanTitleKeyword(p))
        .map((p: string) => p.replace(/^[\s:;,\-–—|]+/, "").trim()) // remove leading punctuation + spaces
        .map((p: string) => p.replace('#', '').trim()) // remove any '#' characters
        .filter(Boolean)

    // Remove duplicates
    filtered = filtered.filter((item: string, idx: any, arr: any[]) =>
        arr.findIndex((i: string) => i.toLowerCase() === item.toLowerCase()) === idx
    );
    ...
    let MakemTitle = (filter: string[] | void[]) => {
...
function FixTableSize() {
    const inputs = getElementAll('input.autoInput');
...
function changePageToClassic() {
    ...
    getElement(".HDRSS").style.opacity = 0;
    getElement(".HDClassic").style.opacity = 1;
...
function openClassic(e: PointerEvent) {
    const changclassListofClassic = (e: { target: { classList: string; }; }) => {
...
async function openRSS(e: PointerEvent) {
    const changePageToRSS = (e: { target: { classList: string; }; }) => {
        e.target.classList = "active Btn";
...
async function buildRSSDom(data: { feeds: any; hermidata: any; }) {
...
function insertRSSPage(dom: { notifications: any; allItems: any; }, {notifSec, allSec}: { notifSec: Element | null; allSec: Element | null; }) {
    try {
...
function makeFooterSection() {
    ...
    clearNotification.addEventListener('click', () => {
        removeAllChildNodes(getElement("#RSS-Notification")) // clear front-end
...
function removeAllChildNodes(parent: { firstChild: any; lastChild: { remove: () => void; }; }) {
...
async function reloadContent(NotificationSection: Element | null,AllItemSection: Element | null) {
...
function makeSortSection(sortSection: Element | null) {
    // makeSortHeader(sortSection);
    makeSortOptions(sortSection);
    sortOptionLogic(sortSection);
}

function setupSearchBar(e_: KeyboardEvent, suggestionBox: HTMLDivElement) {
...
function handleSearchInput(e: Event, suggestionBox: HTMLDivElement) {
...
function applySearchSelection(input: Element | null, suggestionBox: { innerHTML: string; }, value: string) {
    input.value = value;
    filterEntries(value);
    suggestionBox.innerHTML = '';
...
function updateHighlightedSuggestion(items: any[], selectedIndex: number) {
    items.forEach((el: { classList: { toggle: (arg0: string, arg1: boolean) => void; }; }, i: any) => {
        el.classList.toggle('highlighted', i === selectedIndex);
    });
}

function filterEntries(query: string, filtered = null) {
    const allItems = getElementAll('.RSS-entries-item');
    ...
    allItems.forEach(item => {
        ...
        ? filtered.some((f: { title: string; }) => f.title.toLowerCase() === titleText)
...
async function sortOptionLogic(parent_section: { querySelectorAll: (arg0: string) => any; }) {
    ...
    for (const cb of checkboxes) {
        ...
        cb.addEventListener("click", () => {
            ...
            if (section === "Sort") {
                ...
                sortCheckboxes.forEach((otherCb: { dataset: { state: string; }; }) => otherCb.dataset.state = "0");
            ...
            filters.include[section] = filters.include[section].filter((v: any) => v !== label);
            filters.exclude[section] = filters.exclude[section].filter((v: any) => v !== label);
    ...
    const makeActiveState = (cb: { nextElementSibling: { textContent: string; }; closest: (arg0: string) => { (): any; new(): any; firstChild: { (): any; new(): any; textContent: string; }; }; }) => {
        const label = cb.nextElementSibling?.textContent?.trim();
        const section = cb.closest(".filter-section")?.firstChild?.textContent?.trim();
        let state = 0;
    ...
    const hasAnyFilters = (filters: { include: any; exclude: any; sort: any; }) => {
        return (
...
function applySortToEntries(sortType: string) {
    ...
    const getData = (entry: { className: string; }) => {
        const hash = entry.className.split('TitleHash-')[1]?.replace(' seachable','');
        return AllHermidata[hash] || {};
    };

    const compareAlphabet = (a: Element, b: Element, reverse = false) => {
    ...
    const compareDate = (a: Element, b: Element, key: string, reverse = false) => {
...
function getYearNumber(dateInput: string | number | Date){
...
function applyFilterToEntries(filters: { include: ArrayLike<unknown> | { [s: string]: unknown; }; exclude: ArrayLike<unknown> | { [s: string]: unknown; }; }) {
    const entries = getElementAll(".RSS-entries-item");
...
function makeSortOptions(parent_section: { appendChild: (arg0: HTMLDivElement) => void; }) {
    ...
    const createFilterSection = (title: string | null, items: any[], className: string) => {
        ...
        items.forEach((itemText: string | null) => {
 ...
 */
function getYearBucket(dateInput: any) {
...
function makeFeedHeader(parent_section: Element) {
...
function makeItemHeader() {
    if (getElement('.containerHeader-item')) return
    const container = document.createElement('div');
...
async function getLocalNotificationItem(key: string) {
    return new Promise((resolve, reject) => {
        ext.storage.local.get("clearedNotification", (result) => {
            if (ext.runtime.lastError) return reject(new Error(ext.runtime.lastError));
...
async function setNotificationList(key: string | number | null | undefined, value = true) {
    return new Promise((resolve, reject) => {
        if (key === undefined) {
...
function getChapterFromTitleReturn(correctTitle: string, title: string, chapter: number | undefined, url: string) {
...
async function makefeedItem(feedListLocal: ArrayLike<unknown> | { [s: string]: unknown; }, isRSSItem = false) {
...
async function attachEventListeners() {
    // parents
    const notificationFeed = getElementAll('.RSS-Notification-item');
    const allItems = getElementAll('.RSS-entries-item');
...
function clickOnItem(value: { rss: { latestItem: string; }; url: any; }, isRSSItem: boolean) {
...
function rightmouseclickonItem(e: Event, isRSSItem: boolean) {
    e.preventDefault(); // stop the browser’s default context menu
    if (getElement('.feed-header-symbol').dataset.feedState === 'up' && !isRSSItem) return;

    // Remove any existing custom menu first
    getElementAll(".custom-context-menu").forEach(el => el.remove());
...
function copyTitle(target: { className: any; }) {
...
function openInPage(target: { dataset: { url: any; }; }) {
    const url = target.dataset.url;
    if (url) window.open(url, "_self");
}

function openInNewWindow(target: { dataset: { url: any; }; }) {
    const url = target.dataset.url;
    if (url) window.open(url, "_blank");
}

function clearNotification(target: any) {
...
async function addAltTitle(target: any) {
...
async function appendAltTitle(newTitle: string, entry: Hermidata) {
    // Normalize and deduplicate
    const trimmed = trimTitle(newTitle);
    entry.meta = entry.meta || {};
    entry.meta.altTitles = Array.from(
...
async function remove(target: any) {
    const item = getEntriesItem(target)
    if (!item) {
        console.log('isn\'t a entries item');
        return;
...
async function unsubscribe(target: any) {
...
function getEntriesItem(el: { parentElement: { id: string; }; className: string; }) {
...
function getNotificationItem(el: { parentElement: { id: string; }; className: string; }) {
    if (!el) return undefined
...
async function updateFeed(feed: Hermidata, allFeeds: ArrayLike<unknown> | { [s: string]: unknown; }) {
 ...
 * }}
 */
function makeHermidataV3(title: any, url: string | URL, type = "Manga") {
...
async function saveHermidataV3(entry: { id: string; title: any; type: any; meta: { updated: string; }; }) {
...
async function updateChapterProgress(title: any, type: any, newChapterNumber: number) {
    ...
    if (newChapterNumber >= entry.chapter.current) {
        ...
        entry.meta.tags = Array.isArray(rawTags) ? rawTags : rawTags.split(',').map((tag: string) => tag.trim()).filter(Boolean);
...
*/
async function linkRSSFeed(title: string, type: string | undefined, rssData: unknown) {
 ...
 */
async function autoMergeDuplicate(idA: string, idB: string) {
 ...
 * @param {String} id2 
 */
async function SelectDuplicates(id1: string | number, id2: string | number) {
    const data = AllHermidata || await getAllHermidata();
    const obj1 = data[id1];
    const obj2 = data[id2];
 ...
 * }>}
 */
async function findDuplicatescore(id1: string | number, id2: string | number) {
    const data = AllHermidata || await getAllHermidata();
 ...
 * }>}
 */
async function detectAltTitleNeeded(title: string, type: string, source = HermidataV3.source, threshold = 0.85) {
    const data = AllHermidata || await getAllHermidata();
...
function CalcDiff(a: string | number, b: string | number) {
    if (!a || !b) return 0;
    ...
    // Normalize text
    const clean = (str: string) => str.toLowerCase().replace(/[^a-z0-9\s]/gi, '').trim();
    ...
    const common = wordsA.filter((w: any) => wordsB.includes(w)).length;
