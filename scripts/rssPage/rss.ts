...
class RssPage {
    ...
    private openDetails(id: string){
        ...
        if (!hist) return;
        hist.innerHTML = '';
        (item.chapter.bookmarks[item.chapter.bookmarkInUse].history || []).slice().reverse().forEach(h=>{
            const li = document.createElement('li');
            li.textContent = `ch ${h}`;
            hist.appendChild(li);
        });
    ...
    private async saveDetails(){
        ...
        const prev = item.chapter.bookmarks[item.chapter.bookmarkInUse].current;
        const newCurrent = Number(getElement<HTMLInputElement>('#detailCurrent')?.value) || 0;
        if(newCurrent > prev){
        item.chapter.bookmarks[item.chapter.bookmarkInUse].history = [...(item.chapter.bookmarks[item.chapter.bookmarkInUse].history||[]), prev];
        }
